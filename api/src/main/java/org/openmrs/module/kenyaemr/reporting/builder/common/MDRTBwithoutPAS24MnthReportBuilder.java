...
public class MDRTBwithoutPAS24MnthReportBuilder extends AbstractReportBuilder{
	...
	protected List<Mapped<DataSetDefinition>> buildDataSets(ReportDescriptor descriptor, ReportDefinition report) {
		return Arrays.asList(
				...
				ReportUtils.map(createTbOtherCategorywithoutpaspatientwithXpertORconventionalDst(), "startDate=${startDate},endDate=${endDate}"),
	...
	private DataSetDefinition createTbsmearpositiveculturepositivewithoutpaspatientwithXpertORconventionalDst() {
		...
		dsd.setName("A");
		dsd.setDescription("Outcome of patient treated with MDR TB standard regimen without PAS  and with smear positive and Solid OR Liquidculture positive at iniatiation withXpertORconventionalDst");
	...
	private DataSetDefinition createTbsmearnegativeculturpositiveewithoutpaspatientwithXpertORconventionalDst() {
		CohortIndicatorDataSetDefinition dsd = new CohortIndicatorDataSetDefinition();
		dsd.setName("B");
		dsd.setDescription("Outcome of patient treated with MDR TB standard regimen without PAS  and with smear negaitive and Solid OR Liquidculture positive at iniatiation withXpertORconventionalDst");
		dsd.addParameter(new Parameter("startDate", "Start Date", Date.class));
		dsd.addParameter(new Parameter("endDate", "End Date", Date.class));
		dsd.addDimension("parameter",map(commonDimensions.treatmentOutcome()));
		dsd.addDimension("patienttype",map(commonDimensions.typeOfPatients()));
	...
	private DataSetDefinition createTbCategoryIIFwithoutpaspatientwithXpertORconventionalDst() {
		CohortIndicatorDataSetDefinition dsd = new CohortIndicatorDataSetDefinition();
		dsd.setName("C");
		dsd.setDescription("Outcome of patient treated with MDR TB standard regimen without PAS  and with Treatment after failure of treatment (IR , RR) withXpertORconventionalDst");
		dsd.addParameter(new Parameter("startDate", "Start Date", Date.class));
		dsd.addParameter(new Parameter("endDate", "End Date", Date.class));
		dsd.addDimension("parameter",map(commonDimensions.treatmentOutcome()));
		dsd.addDimension("patienttype",map(commonDimensions.typeOfPatients()));
		...
		EmrReportingUtils.addRow(dsd, "CA1", "No. of detected cases with Treatment After Failure(IR or RR) with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "CB1", "No. of detected cases with Treatment After Failure(IR or RR) with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "CC1", "No. of detected cases with Treatment After Failure(IR or RR)  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "CD1", "No. of detected cases with Treatment After Failure(IR or RR)  with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "CE1", "No. of detected cases with Treatment After Failure(IR or RR)  with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "CF1", "No. of detected cases with Treatment After Failure(IR or RR)  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "CG1", "No. of detected cases with Treatment After Failure(IR or RR)  with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
	...
	private DataSetDefinition createTbCategoryIITADwithoutpaspatientwithXpertORconventionalDst() {
		...
		EmrReportingUtils.addRow(dsd, "DA1", "No. of detected cases with Treatment after lost to follow up (IR , RR) with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "DB1", "No. of detected cases with Treatment after lost to follow up (IR , RR) with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "DC1", "No. of detected cases with Treatment after lost to follow up (IR , RR)  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "DD1", "No. of detected cases with Treatment after lost to follow up (IR , RR)  with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "DE1", "No. of detected cases with Treatment after lost to follow up (IR , RR)  with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "DF1", "No. of detected cases with Treatment after lost to follow up (IR , RR)  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "DG1", "No. of detected cases with Treatment after lost to follow up (IR , RR)  with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsG);
		return  dsd;
	}
	...
	private DataSetDefinition createTbRelapseCategoryIIwithoutpaspatientwithXpertORconventionalDst() {
		...
		EmrReportingUtils.addRow(dsd, "EA1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "EB1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "EC1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "ED1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "EE1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "EF1", "No. of detected cases with Relapse ( IR, RR)  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "EG1", "No. of detected cases with Relapse ( IR, RR)  with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
	...
	private DataSetDefinition createTbCategoryIFwithoutpaspatientwithXpertORconventionalDst() {
		...
		EmrReportingUtils.addRow(dsd, "FA1", "No. of detected cases with Non-Converter (IR , RR)  with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "FB1", "No. of detected cases with Non-Converter (IR , RR) with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "FC1", "No. of detected cases with Non-Converter (IR , RR)  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "FD1", "No. of detected cases with Non-Converter (IR , RR) with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "FE1", "No. of detected cases with Non-Converter (IR , RR) with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "FF1", "No. of detected cases with Non-Converter (IR , RR) with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "FG1", "No. of detected cases with Non-Converter (IR , RR) with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
	private DataSetDefinition createTbNewCategorywithoutpaspatientwithXpertORconventionalDst() {
		...
		EmrReportingUtils.addRow(dsd, "HA1", "No. of detected cases with NEW patient tb cases  with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "HB1", "No. of detected cases with NEW patient tb cases with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "HC1", "No. of detected cases with NEW patient  tb cases  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "HD1", "No. of detected cases with NEW patient tb cases with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "HE1", "No. of detected cases with NEW patient tb cases with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "HF1", "No. of detected cases with NEW patient tb cases with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "HG1", "No. of detected cases with NEW patient tb cases with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
	private DataSetDefinition createTbMDRTBCASESwithoutpaspatientwithXpertORconventionalDst() {
		...
		EmrReportingUtils.addRow(dsd, "GA1", "No. of detected cases with MDR tb cases  with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "GB1", "No. of detected cases with MDR tb cases with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "GC1", "No. of detected cases with MDR tb cases  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "GD1", "No. of detected cases with MDR tb cases with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "GE1", "No. of detected cases with MDR tb cases with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "GF1", "No. of detected cases with MDR tb cases with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "GG1", "No. of detected cases with MDR tb cases with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
       
	
	private DataSetDefinition createTbOtherCategorywithoutpaspatientwithXpertORconventionalDst() {
		CohortIndicatorDataSetDefinition dsd = new CohortIndicatorDataSetDefinition();
		dsd.setName("I");
		...
		EmrReportingUtils.addRow(dsd, "IA1", "No. of detected cases with Other Category cases  with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "IB1", "No. of detected cases with Other Category cases with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "IC1", "No. of detected cases with Other Category cases  with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "ID1", "No. of detected cases with Other Category with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "IE1", "No. of detected cases with Other Category cases with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "IF1", "No. of detected cases with Other Category cases with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "IG1", "No. of detected cases with Other Category cases with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsG);
		return  dsd;
	}
		...
		EmrReportingUtils.addRow(dsd, "JA1", "No. of detected cases with smear (+)ve and Culture (+) with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "JB1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "JC1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "JD1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "JE1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "JF1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "JG1", "No. of detected cases new with smear (+)ve and Culture (+) with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.Tbsmearculturewithoutpas(), indParams), columnsG);
		return  dsd;
	}
	private DataSetDefinition createTbsmearnegativeculturpositiveewithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "KA1", "No. of detected cases with smear (-)ve and Culture (+) with outcome (Cure) ", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsA);
		EmrReportingUtils.addRow(dsd, "KB1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (Died) ", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsB);
		EmrReportingUtils.addRow(dsd, "KC1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (Completed)", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsC);
		EmrReportingUtils.addRow(dsd, "KD1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (Failure)", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsD);
		EmrReportingUtils.addRow(dsd, "KE1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (Defaulted)", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsE);
		EmrReportingUtils.addRow(dsd, "KF1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsF);
		EmrReportingUtils.addRow(dsd, "KG1", "No. of detected cases new with smear (-)ve and Culture (+) with outcome (Still Enroll)", ReportUtils.map(tbIndicatorLibrary.Tbsmearnegativeculturewithoutpas(), indParams), columnsG);
		return  dsd;
	}
	private DataSetDefinition createTbNewCategorywithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "LF1", "No. of detected cases with New category  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmednewCategoryMDRTBwithoutpas(), indParams), columnsF);
	...
	private DataSetDefinition createTbCategoryIIFwithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "MF1", "No. of detected cases with Treatment after failure of treatment (IR , RR)  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedCatIITBwithoutpas(), indParams), columnsF);
	...
	private DataSetDefinition createTbCategoryIITADwithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "NF1", "No. of detected cases with Treatment after lost to follow up (IR , RR) with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedDefaultwithoutpasTB(), indParams), columnsF);
	...
	private DataSetDefinition  createTbRelapseCategoryIIwithoutpaspatientwithTBEnrollNumber(){
		...
		EmrReportingUtils.addRow(dsd, "OF1", "No. of detected cases with Relapse ( IR, RR)  with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedTBwithoutpas(), indParams), columnsF);
	...
	private DataSetDefinition createTbCategoryIFwithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "PF1", "No. of detected cases with Non-Converter (IR , RR) with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedCatITBwithoutpas(), indParams), columnsF);
	...
	private DataSetDefinition createTbMDRTBCASESwithoutpaspatientwithTBEnrollNumber() {
		CohortIndicatorDataSetDefinition dsd = new CohortIndicatorDataSetDefinition();
		dsd.setName("Q");
		dsd.setDescription("Outcome of patient treated with MDR TB standard regimen without PAS  and with MDR tb cases withTBEnrollNumber from Enrollment Form");
		dsd.addParameter(new Parameter("startDate", "Start Date", Date.class));
		dsd.addParameter(new Parameter("endDate", "End Date", Date.class));
		dsd.addDimension("parameter",map(commonDimensions.treatmentOutcome()));
		dsd.addDimension("patienttype",map(commonDimensions.typeOfPatients()));
		...
		EmrReportingUtils.addRow(dsd, "QF1", "No. of detected cases with MDR tb cases with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedstandardMDRTBwithoutpas(), indParams), columnsF);
	...
	private DataSetDefinition createTbOtherCategorywithoutpaspatientwithTBEnrollNumber() {
		...
		EmrReportingUtils.addRow(dsd, "RF1", "No. of detected cases with Other Category cases with outcome (TransferedOut)", ReportUtils.map(tbIndicatorLibrary.confirmedotherCategoryMDRTBwithoutpas(), indParams), columnsF);
